...
const MODE_OPTIONS: Array<{ value: ServiceMode; label: string }> = [
  ...
  { value: "blood_bank", label: "Blood requirement" },
...
export function AIModeSelector({ value, onChange }: AIModeSelectorProps) {
  return (
    <label className="humangrid-ai-mode-selector">
      ...
      <select value={value} onChange={(event) => onChange(event.target.value as ServiceMode)} aria-label="Select emergency category">
